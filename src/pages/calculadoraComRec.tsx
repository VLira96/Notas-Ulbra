        ...
        <form className="text-base flex flex-col gap-24 items-center m-16" onSubmit={printaNota}>
            <section className='lg:gap-48 flex flex-col lg:flex-row items-center'>
                <div className="py-10">
                    ...
                    <input className='rounded text-black text-base' type="number" onChange={(evento) => setG1(Number(evento.target.value))} id='g1' step={0.1} max={10} min={0} />
                </div>

                <div className="py-10">
                    <label htmlFor="numero">Nota de Grau 2: </label>
                    <input className='rounded text-black text-base' type="number" onChange={(evento) => setG2(Number(evento.target.value))} id='g2' step={0.1} max={10} min={0} />
                </div>

                <div className="py-10">
                    <label htmlFor="numero">Nota da Recuperação: </label>
                    <input className='rounded text-black text-base' type="number" onChange={(evento) => setRec(Number(evento.target.value))} id='rec' step={0.1} max={10} min={0} />
                </div>
            </section>

            {precionado && g1 <= 10 && g1 >= 0 && g2 <= 10 && g2 >= 0 && rec <= 10 && rec >= 0 && <MostraNota nota={Number(nota.toFixed(1))} />}
